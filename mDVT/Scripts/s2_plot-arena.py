...
video_dir = 'D:\\GitHub_Res\\FlyBehavioralAnalysis\\mDVT\Data\\ClippedVideo\\w1118-Rep2'
...
for _, A_entry in meta_df.iterrows():
    ...
    print("Processing file: " + position_filepath)
    if not os.path.exists(position_filepath):
        print('Sample Escaped: ', A_entry['videoname'])
        continue
    arena_and_trace_plot = f"{dvt_output_filepath}/{A_entry['videoname']}_motion_trace_circle.png"
    arena_plot = f"{dvt_output_filepath}/{A_entry['videoname']}_raw.png"
    if os.path.exists(arena_plot) and os.path.exists(arena_and_trace_plot):
        continue
    position_df = pd.read_csv(position_filepath)
    # blank_image = np.zeros((768, 1024, 3), np.uint8)
    # blank_image[:, :] = (255, 255, 255)
    video_filepath = os.path.join(video_dir,A_entry['videoname'])
    ...
    cv2.imencode('.jpg', blank_image)[1].tofile(arena_plot)
    ...
    cv2.imencode('.jpg', blank_image)[1].tofile(arena_and_trace_plot)
    cap.release()
